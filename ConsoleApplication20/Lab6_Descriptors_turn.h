...
public:
	...
	vector<InterestingPoint*>* getInterestingPointList() {
	...
	void setSizeRegion(int sizeRegion) {
	 ...
	 */
	Lab6_Descriptors_turn(vector<pair<int, int>*>* listPoints, IMG* img_Atan, IMG* img_gradient, int kek) {
		this->kek = kek;
...
//        sum = sqrt(sum);
	...
	static vector<pair<InterestingPoint*, InterestingPoint*>*>* createPairs(
		Lab6_Descriptors_turn* first, Lab6_Descriptors_turn* second) {
		...
		for (int i = 2; i < pairsList->size(); i++) {
			...
			indexOk = i;
			if (pairsList->at(i)->second > Lab6_Descriptors_turn::LEVEL)
				break;
		}
	...
	static vector<pair<InterestingPoint*, InterestingPoint*>*>* createPairs(
		vector<InterestingPoint*>* first, vector<InterestingPoint*>* second)
	{
		vector<pair<pair<InterestingPoint*, InterestingPoint*>*, double>*>* pairsList
			= new vector<pair<pair<InterestingPoint*, InterestingPoint*>*, double>*>;
		...
		for (int i = 0; i < first->size(); i++) {
			...
			// один из дескрипторов первого изоб
			InterestingPoint* firstInterestingPoint = first->at(i);
			InterestingPoint* secondInterestingPoint = NULL;
			int indexSecond = -1;
			double distance = 9999999.0;
			...
			for (int j = 0; j < second->size(); j++) {
				// дескриптор интересной точки второго изображения.
				secondInterestingPoint = second->at(j);
				InterestingPoint* tmpInterestingPoint = secondInterestingPoint;
	...
	static IMG* drawLine(
		...
		for (int i = 0; i < pairs->size(); i++) {
			...
			if (IS_COLOR_LINE) {
				...
				line(
					tmp_image,
					Point2f(first->realCol, first->realRow),
					Point2f(second->realCol + firstImg->width, second->realRow),
					Scalar(colors[0], colors[1], colors[2])
				);
				delete[] colors;
			}
...
double Lab6_Descriptors_turn::LEVEL = 0.05;
int Lab6_Descriptors_turn::count_nndrOk = 0;
bool Lab6_Descriptors_turn::IS_COLOR_LINE = true;